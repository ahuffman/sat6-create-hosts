...
|manage_host_build|no|When using `register_only: True` and `force_host_update: True`, will allow for the Host's build cycle to become managed by Satellite6 and Virtual Machine Power if it is utilizing a compute resource. Defaults to `False`.
